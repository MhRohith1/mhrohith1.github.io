...
- a rank $(r,s)$ tensor is a map $\boldsymbol T$ such that $$\boldsymbol T: \underbrace{V \otimes V \otimes \ldots \otimes V }_{s\text{ inputs}} \otimes \underbrace{V^* \otimes V^* \otimes \ldots \otimes V^*}_{r \text{ inputs}} \rightarrow \mathbb R \text{ or } \mathbb C$$
...
### Examples
- The metric tensor $\mathbf g$ is a rank $(0,2)$ tensor. An example is the Minkowski metric: $$\eta_{\alpha \beta}A^\alpha B^\beta := \vec{A} \cdot \vec{B}$$
