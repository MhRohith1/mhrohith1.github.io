...
- In Electrodynamics, the Lagrangian can be represented as a rank-2 antisymmetric tensor. in class, we dealt with it as $$L=K-q \boldsymbol{A}(\boldsymbol r, t) \cdot \dot{\boldsymbol{r}} - q \varphi(\boldsymbol r, t)$$
- In General Relativity, the Einstein-Hilbert action has the Lagrangian $$L=\frac{1}{2\kappa} R \sqrt{-g}$$where $R$ is the Ricci Scalar and $\kappa = 8\pi G c^{-4}$, and $g$ the metric tensor. Minimizing this Lagrangian yields the Einstein Field Equations.
